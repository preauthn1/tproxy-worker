...
function relayBatch(value){const view=new DataView(value);let offset=0;while(offset<value.byteLength){if(value.byteLength-offset<8)return fail();const type=view.getUint8(offset),streamId=(view.getUint8(offset+1)<<16)|(view.getUint8(offset+2)<<8)|view.getUint8(offset+3),size=view.getUint32(offset+4),end=offset+8+size;if(size>1048576||end>value.byteLength)return fail();if(type===5&&streamId===0){const pong=new Uint8Array(value.slice(offset,end));pong[0]=6;send(pong.buffer)}else{const frame=value.slice(offset,end);port.postMessage({t:'traffic',up:0,down:frame.byteLength});port.postMessage(frame,[frame])}offset=end}if(offset!==value.byteLength)fail()}
async function create(first){try{status('connecting');const response=await fetch(relayOrigin+'/api/v1/session',opts('POST',bootstrap,first));if(response.status!==200||response.headers.get('X-Carrier-Mode')!==carrierMode)throw 0;sessionToken=response.headers.get('X-Session-Token')||'';if(!sessionToken)throw 0;const welcome=await response.arrayBuffer();socket=new WebSocket(relayOrigin.replace(/^https:/,'wss:')+'/api/v1/ws','tproxy-v1.'+sessionToken);socket.binaryType='arraybuffer';socket.onmessage=event=>{if(!(event.data instanceof ArrayBuffer)||!event.data.byteLength)return fail();relayBatch(event.data)};socket.onclose=()=>{if(!closed)fail()};socket.onerror=()=>{};socket.onopen=()=>{if(closed)return socket.close();port.postMessage(welcome,[welcome]);status('connected');for(const value of queued.splice(0)){queuedBytes-=value.byteLength;send(value)}}}catch{fail()}}
